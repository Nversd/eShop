...
function addToCart(goodsId, parentId,is_ajax)
{
	...
	if(is_ajax == 1){
		Ajax.call('flow.php?step=add_to_cart', 'goods=' + objToJSONString(goods), addToCartResponse_ajax, 'POST', 'JSON');
	}else{
		Ajax.call('flow.php?step=add_to_cart', 'goods=' + objToJSONString(goods), addToCartResponse, 'POST', 'JSON');
	}
}
...
/* add xhb*/
	if(result.error==10){
		...
		//&back_act=http://www.zhengou.com/goods-398.html
	...
	else
	{
		...
		else
		{
			...
			case '2' :
				...
				break;
			case '3' :
				Ajax.call('flow.php?step=insert_cart_info_1', '', function(result){
						document.getElementById('cart_info_1').innerHTML = result.content;
				}, 'POST', 'JSON');
				//更新header上面的购物车

				divTipmiddle(result);//显示层
				//refresh_cart();
...
goods.number   = number;
goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
Ajax.call('flow.php?step=add_to_cart1', 'goods=' + objToJSONString(goods) , addToCartResponse2, 'POST', 'JSON');
}
