		...
		this.each(function(){
			...
			$this.find("[datatype]").blur(function(){
				...
				if(!flag){
						...
						$.ajax({
							...
							success: function(s){
								if(s=="true"){
								    ...
								    $.fn.Validform.sn.showmsg(tipmsg.r,settings.tiptype,{obj:inputobj,type:2});
								}else{
									if(s=="false") s="该用户名已被使用，请使用其它用户名注册";
					...
					}else if($(this).attr("ajaxemailurl")){//检测邮箱
						var inputobj=$(this);
						inputobj.attr("valid",tipmsg.c);
						$.fn.Validform.sn.showmsg(tipmsg.c,settings.tiptype,{obj:inputobj,type:1},"hide");
						$.ajax({
							type: "get",
							url: inputobj.attr("ajaxemailurl"),
							data: "email="+$(this).val(),
							dataType: "text",
							success: function(s){
								if(s=="ok"){
